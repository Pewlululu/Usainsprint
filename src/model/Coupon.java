...
public class Coupon {
    ...
    public static ArrayList<Coupon> Search(String txtString) {
        String ConTxt = "%" + txtString + "%";

        ArrayList<Coupon> arrayList = new ArrayList<>();
        try {
            ResultSet rs = DbOperations.getData("select * from coupon where CouponName like '" + ConTxt + "' ");
            while (rs.next()) {
                Coupon coupon = new Coupon();
                coupon.setCouponId(rs.getInt("CouponId"));
                coupon.setCouponName(rs.getString("CouponName"));
